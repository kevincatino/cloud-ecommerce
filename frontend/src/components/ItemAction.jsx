...
const ActionButton = ({ 
    ...
    return (
        <div className={`flex flex-col items-center justify-start ${hasBorderRight ? 'border-r-2 border-white' : ''} ${hasBorderLeft ? 'border-l-2 border-white' : ''}`} style={{ width: '33.33vw', height: '100%' }}>
        <button
        ...
        {hasInput && (
            Object.keys(inputValue).map(k =>
                <input 
                    ...
                    type="text" 
                    placeholder={placeholder + " " + k.replace(/([A-Z])/g, ' $1').toLowerCase()}
                    value={inputValue[k]}
